...
export class SearchFilterComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.searchService.doSilentApproximativeSearchWithCurrentFilter();

    // Component -> State
    this.form.valueChanges.pipe(takeUntil(this.unsubscribe$)).subscribe({
      next: (filterValues) => {
        console.log('Component -> State : ', filterValues);
        this.searchService.updateFilter(filterValues);
      }
    });

    // State -> Component
    this.searchService.currentSearchFilter$.pipe(takeUntil(this.unsubscribe$)).subscribe({
      next: (filterValues) => {
  ...
  sendSearchFilter(): void {
    ...
    this.router.navigate(['annonces/recherche'], { queryParams: { ...filter } });
  }
