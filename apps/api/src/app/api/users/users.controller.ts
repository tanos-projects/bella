...
export class UsersController {
  ...
  getProfile(@Request() req): Observable<UserDTO> {
    const user: AuthUser = req.user;
    return this.usersService.findOneByIdpId(user.sub).pipe(map(modelToDTO));
  }
  ...
  findOne(@Param('id') id: string): Observable<UserDTO> {
    return this.usersService.findOne(id).pipe(map(modelToProfileDTO));
  }
